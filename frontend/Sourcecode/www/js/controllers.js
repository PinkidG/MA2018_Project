...
//========================================
  .controller('homeCtrl',
    ...
    function ($scope, $state, sharedProperties, sharedParameter, TopicService, checkPlatform, UserService, $mdDialog, $ionicLoading) {
      ...
      // Refresh information about user
      ...
      // Check if deep link was used (and forward if needed)
  ...
  .controller('loginCtrl',
    function ($scope, $ionicPlatform, AuthService, UserService, checkPlatform, sharedProperties, $state, $mdDialog, $location) {
      ...
      // Check if we are on iOS and TouchID (or FaceID) is available
      ...
      $scope.login = function (ev) {
        AuthService.login($scope.user).then(function (user) {
          ...
          UserService.refreshUser(user.userId).then(function (ruser) {
            ...
            if (!checkPlatform.isBrowser()) {
              navigator.notification.confirm(errMsg.data, function (buttonIndex) {}, "Server-Fehler", ["Erneut versuchen"]);
            } else {
              let confirm = $mdDialog.alert()
                .title('Server-Fehler')
          ...
          if (!checkPlatform.isBrowser()) {
            navigator.notification.confirm("Bitte Benutznamen und Passwort überprüfen.", function (buttonIndex) {}, "Benutzer-Fehler (1M)", ["Erneut versuchen"]);
          } else {
            let confirm = $mdDialog.alert()
  ...
  //========================================
  // Registration controller for patients
  //========================================
  ...
  //========================================
  // Registration controller for doctors
  //========================================
    ...
    function ($scope, $state, sharedProperties, checkPlatform, $mdDialog, UserService, $ionicLoading) {
      ...
      $scope.save = function () {
        ...
        UserService.updateUser($scope.user).then(function (user) {
          sharedProperties.setProperty(user);
          if (!checkPlatform.isBrowser()) {
            navigator.notification.confirm("Update erfolgreich!", function (buttonIndex) {}, "Erfolg", ["Okay"]);
          } else {
            var confirm = $mdDialog.alert()
              .title('Erfolg')
            ...
            switch (buttonIndex) {
              case 1:
                UserService.deleteUser().then(function (msg) {
                  navigator.notification.confirm("Löschen erfolgreich!", function (buttonIndex) {}, "Erfolg", ["Okay"]);
                  $state.go('login');
                }, function (errMsg) {
                  navigator.notification.confirm(errMsg.statusText, function (buttonIndex) {}, "Fehler", ["Erneut versuchen"]);
                })
